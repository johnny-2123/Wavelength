...
import {
  fetchGamesBetweenFriends,
  fetchFriendDetails,
  fetchWordsBetweenFriends,
...
const FriendDetails = ({ sendMessage, sessionUser }) => {
  ...
  const friendNotification = (message) =>
    toast(message, {
  ...
  const numGamesWon = games?.reduce((acc, game) => {
    ...
    if (lastRound?.user1Agrees && lastRound?.user2Agrees) {
      ...
      lastRoundWords &&
      lastRoundWords.length >= 2 &&
      lastRoundWords[0].wordText === lastRoundWords[1].wordText
    ) {
      return acc + 1;
    } else {
      return acc;
  ...
  useEffect(() => {
    dispatch(fetchFriendDetails(friendId))
      ...
      .catch((err) => {});
    dispatch(fetchWordsBetweenFriends(friendId)).catch((err) => {});
    ...
    datasets: [
      {
        ...
        backgroundColor: ["#3B9778", "#D85F56"],
        borderColor: ["#3B9778", "#D85F56"],
        borderWidth: 3,
      },
    ],
  };
    ...
    <div className="mainFriendDetailsDiv">
      {friend?.status === "accepted" && (
        <>
          <div className="friendDetailsTopDiv">
            <div className="friendDetailsTopLeftDiv">
              ...
              <button
                ...
                className="friendDetailsButton"
              >
                Remove Friend <i class="fa-solid fa-user-minus"></i>
              </button>
            </div>
          ...
          <Slider className="gamesForFriendSlider" {...settings}>
            {gamesMapped}
          </Slider>
