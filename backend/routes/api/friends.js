...
router.get("/top", requireAuth, async (req, res) => {
  ...
  const sortedFriends = Object.keys(gamesWithFriends).sort(
    (a, b) => gamesWithFriends[b] - gamesWithFriends[a]
  );
...
router.get("/:friendId/games", requireAuth, async (req, res) => {
  ...
  const { friendShipSent, friendShipReceived } = await findFriendship(
    userId,
    friendId
  );

  const validationError = validateFriendship(
    userId,
    friendShipSent,
    friendShipReceived
  );
  if (validationError)
    return res
      ...
      .json({ errors: validationError.message });
...
router.get("/:friendId", requireAuth, async (req, res) => {
  ...
  const friend = friendShipSent
    ? friendShipSent?.ReceivingUser
    : friendShipReceived?.RequestingUser;
...
router.delete("/:friendId", requireAuth, async (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Friendship deleted.", friendship: friendshipToDelete });
});
...
router.put("/:friendId", requireAuth, async (req, res) => {
  ...
  if (!friend) return res.status(404).json({ errors: "Friend not found." });
  ...
  if (status === "accepted" && friendShipReceived) {
    ...
    return res
      .status(200)
      .json({
        message: [`Friend request from ${friend.username} accepted.`],
        friendship: friendShipReceived,
      });
  }
...
router.post("/", requireAuth, async (req, res) => {
  ...
  const friend = await User.findOne({
    where: {
      [Op.or]: [{ username: friendCredential }, { email: friendCredential }],
    },
  ...
  const [friendShipSent, friendShipReceived] = await Promise.all([
    ...
    Friend.findOne({
      where: { userId: friend.id, friendId: userId },
      include: includeUsers,
    }),
  ]);
  ...
  if (friendShipSent && friendShipSent.status === "accepted") {
    return res
      .status(400)
      .json({ errors: ["You are already friends with this user"] });
  }
  ...
  res
    .status(201)
    .json({ message: [`Friend request sent to ${friend.username}.`] });
});
